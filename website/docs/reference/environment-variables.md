...
| `HERMES_CRON_MAX_RUNTIME` | Hard wall-clock cap for cron job agent runs in seconds (default: `7200`, 2 hours). Unlike `HERMES_CRON_TIMEOUT` this fires even while the run looks active, so a job stuck retrying failed provider calls cannot run forever (and cannot hold the workdir lock that serializes other cron jobs). Set to `0` for unlimited. Also configurable via `cron.max_runtime_seconds` in `config.yaml`. |
